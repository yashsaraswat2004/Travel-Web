    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                <DashboardCard title="Total Bookings" value="1,234" icon={<Calendar className="h-8 w-8" />} trend="+5.25%" />
                <DashboardCard title="Total Revenue" value="$52,345" icon={<DollarSign className="h-8 w-8" />} trend="+3.75%" />
                <DashboardCard title="Active Users" value="15,400" icon={<Users className="h-8 w-8" />} trend="+2.5%" />
                <DashboardCard title="Popular Destinations" value="25" icon={<Globe className="h-8 w-8" />} trend="+1" />
            </div>
            ...
            <div className="border-t border-gray-200">
                <ul className="divide-y divide-gray-200">
                    {destinations.length > 0 ? (
                        destinations.slice(0, 3).map((result) => (
                            <BookingItem
                                key={result._id}
                ...
                <ul className="divide-y divide-gray-200">
                    {destinations.length > 0 ? (
                        destinations.map((result) => (
                            <DestinationItem key={result._id} name={result.name} country={result.country} bookings={120} revenue="₹ 145,000" />
                        ))
                    ) : (
