...
export default function AddDestination() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const response = await axios.post('http://localhost:5070/api/admin/add-destination', destination, {
        headers: {
  ...
  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
      <div className="px-4 py-5 sm:px-6">
        <h3 className="text-lg leading-6 font-medium text-gray-900">Add New Destination</h3>
        <p className="mt-1 max-w-2xl text-sm text-gray-500">Enter the details for the new destination.</p>
      </div>
      <div className="border-t border-gray-200">
        <form onSubmit={handleSubmit} className="px-4 py-5 sm:p-6">
          <div className="grid grid-cols-6 gap-6">
            <div className="col-span-6 sm:col-span-3">
              <label htmlFor="name" className="block text-sm font-medium text-gray-700">Destination Name</label>
              <input
                type="text"
                name="name"
                id="name"
            ...
            <div className="col-span-6 sm:col-span-3">
              <label htmlFor="pricePerPerson" className="block text-sm font-medium text-gray-700">Price Per Person</label>
              <input
                type="number"
                name="pricePerPerson"
                id="pricePerPerson"
            ...
            <div className="col-span-6">
              ...
              {destination.itinerary.map((day, index) => (
                <div key={index} className="mt-2">
                  <label className="block text-sm font-medium text-gray-700">Day {day.day}</label>
                  <textarea
                    name="activities"
